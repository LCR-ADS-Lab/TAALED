...
"""
version = ".30" #split off from pre-process_17.py
...
class lexdiv():
	...
	def MTLD_MFL(self,windowl,factorprop, listout = False): #window length, factor proportion
		...
		for wl, fp in zip(windowl,factorprop): #iterate through window lengths and factor proportions
			#print(wl/fp)
			if fp == 0:
				continue #skip factors that were not long enough to be counted (less than minimum)
...
def ldwrite(lof,outname = "results.csv", loi = None, sep = "\t", funct = lexdiv, prll = False, params = None, mn = 50, mx = 200, interval = 5, clss = True, functd = None): #presumes that pylats and spacy are installed
	...
	for fname in lof: #iterate through filenames
		...
		if prll == True: #if parallel analysis is conducted
			...
			ldout = funct(normed_text).vald #get dictionary version of lexical diversity output
