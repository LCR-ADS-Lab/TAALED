...
MTLD reports the value of MTLD-bidirectional.
```python
print(ldvals.mtld) #This is MTLD-bi?
```
```result
82.7575
```
...
MTLDO reports the original approach taken by McCarthy and Jarvis (2010). This approach devide the sum of the factor lengths by the partial factor.
```python
print(ldvals.mtldo) #report mtld original
```
```result
81.38912039538282
```
...
MTLDVALS method returns lengths of each factor used for calculation of final the MTLD score. In the following example, the first two factors were 99 and 74 words in lengths.
```python
...
```python
print(ldvals.mtldlists) #should I include this? maybe not necessary, but may still be helpful
```
