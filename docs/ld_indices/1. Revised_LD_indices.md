...
The `.mtldo` method reports the original MTLD as it was explained by McCarthy and Jarvis (2010) in their paper. This approach devide the sum of the factor lengths by the partial factor.
```python
...
The `.mtldvals` method method returns lengths of each factor used for calculation of the final MTLD score. In the following example, the first two factors were 99 and 74 words in lengths.
```python
...
The `.mtldplot` method will return a density plot for factor lengths. This information will tell you the distribution of factor lengths before taking the average.
```python
