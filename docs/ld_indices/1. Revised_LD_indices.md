...
The `.mtld` reports the MTLD value, where the average of the factor lengths is calculated after the final partial factor is modified by the factor proportion.
```python
...
The `.mtldo` method reports the original MTLD as it was explained by McCarthy and Jarvis (2010) in their paper. This approach devide the sum of the factor lengths (including both full and partial factors) by the sum of factor scores (a full factor recieves the score of 1, and the partial factor score ranges from 0 to 1), which can gives you a slightly different results from `.mtld` implementation.
```python
...
The `.mtldvals` method returns lengths of each factor used for calculation of the final MTLD score. In the following example, the first two factors were 99 and 74 words in lengths.
```python
...
The `.mtldplot` method returns a density plot for factor lengths. This information will tell you the distribution of factor lengths before taking the average.
```python
