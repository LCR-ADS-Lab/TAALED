...
The `.mtld` reports the MTLD value, where the average of the factor lengths are calculated. The average is calculated after the final partial factor is modified by the factor proportion.
```python
