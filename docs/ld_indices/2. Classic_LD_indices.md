...
* TTR is perhaps the most well known LD index. Johnson (1939, 1944), in the field of psychology, discussed the type-token ratio (TTR) applying the terms type and token coined by Peirce (1906, pp. 505-506) in order to come up with reliable measures of language behavior (p. 1). Johnson (1944), listed varying statistical or mathematical procedures of TTRs, which are _the ove-all TTR_, the _mean segmental TTR_, and _the cumulative TTR_.
