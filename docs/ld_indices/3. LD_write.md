...
1. When the corpus files successfully processed by TAALED, we will find an output file on the working directory.
...
2. The name of the file is "20220328_002547_sample_nopa.txt" (non-parallel).
...
3. If you want to do further (statistical) analysis with the calculated values, we can copy the output to other program (e.g., Excel).
...
4. While doing the parallel analysis, a text could be skipped if the text length is shorter than the mx number.

(For more information about the parallel analysis, read LEARN MORE below.)
...
5. The name of the file is "20220328_004640_sample_pa.txt" (parallel).
